...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const searchInput = wrapper.querySelector('.phone-input-wrapper__search-input');
  ...
  function renderList(items) {
    ...
    items.forEach(c => {
      ...
      li.addEventListener('click', () => {
        ...
        closeDropdown();
        syncFullPhone();
        if (window.__contactInputModalModeEnabled !== true && window.__contactInputModalOpen !== true) {
          try {
            numberInput.focus({
              preventScroll: true
            });
  ...
  function openDropdown() {
    ...
    if (window.innerWidth > 1140 && window.__contactInputModalModeEnabled !== true) {
  ...
  function syncFullPhone() {
    ...
    fullHidden.dispatchEvent(new Event('input', {
      bubbles: true
    }));
  }
  ...
  searchInput.addEventListener('input', () => {
    ...
    const filtered = COUNTRIES.filter(c => `${c.name} ${c.iso} ${c.dial}`.toLowerCase().includes(q));
    renderList(filtered);
  });
...
//# sourceMappingURL=phone-country-select.js.map