...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const DIAL_CODES_SORTED = Object.keys(PHONE_LENGTHS_BY_DIAL).sort((a, b) => b.length - a.length); // щоб +380 перевірялось раніше за +3 і т.д.
  ...
  .addField('#user-name-contact', [{
    rule: 'required',
    errorMessage: '&#8613 Please enter your name'
  }, {
    validator: value => {
      const v = value.trim();
      return /^\p{L}[\p{L}\p{M}]{1,49}(?:[ '\-’]\p{L}[\p{L}\p{M}]{1,49})*$/u.test(v);
    },
  ...
  .addField('#user-email-contact', [{
    rule: 'required',
    errorMessage: '&#8613 Please enter your email'
  }, {
    rule: 'email',
  ...
  ['#user-name-contact', '#user-email-contact', '#user-tel-contact', '#user-tel-full'].forEach(autofillHackWhileFocused);
  ...
  validator.onSuccess(event => {
    ...
    fetch('send_email.php', {
      ...
      setTimeout(() => {
        thankYouEl.classList.add('contact-section__thank-you-message--visible');
      }, 200);
    }).catch(error => {
