...
(_window$visualViewpor7 => {
  ...
  function normWheelDY(e) {
    ...
    let dy = e.deltaY;
    if (e.deltaMode === 1) dy *= LINE;else if (e.deltaMode === 2) dy *= PAGE;
    return dy;
  }
  ...
  function idHashForIndex(i) {
    ...
    const el = sections[i];
    return el !== null && el !== void 0 && el.id ? `#${el.id}` : '';
  }
  ...
  function getDeviceSizeKey() {
    var _window$screen, _window$screen2, _window$screen3;
    const sw = ((_window$screen = window.screen) === null || _window$screen === void 0 ? void 0 : _window$screen.width) || 0;
    const sh = ((_window$screen2 = window.screen) === null || _window$screen2 === void 0 ? void 0 : _window$screen2.height) || 0;
    const o = ((_window$screen3 = window.screen) === null || _window$screen3 === void 0 || (_window$screen3 = _window$screen3.orientation) === null || _window$screen3 === void 0 ? void 0 : _window$screen3.type) || '';
  ...
  function getFixedIndexForResize() {
    var _stops$hashIdx, _stops$currentIdx;
    const currentIdx = clamp(current, 0, sections.length - 1);
    const id = (location.hash || '').trim();
    if (id.length <= 1) return currentIdx;
    const target = document.querySelector(id);
    const sec = (target === null || target === void 0 ? void 0 : target.closest('.section')) || target;
    const hashIdx = sections.indexOf(sec);
    if (hashIdx < 0) return currentIdx;
    if (!stops.length) return hashIdx;
    const y = Math.round(window.scrollY);
    const dHash = Math.abs(((_stops$hashIdx = stops[hashIdx]) !== null && _stops$hashIdx !== void 0 ? _stops$hashIdx : 0) - y);
    const dCurrent = Math.abs(((_stops$currentIdx = stops[currentIdx]) !== null && _stops$currentIdx !== void 0 ? _stops$currentIdx : 0) - y);
  ...
  function shouldPauseFullpage() {
    return isContactInputModalOpen() || isPhoneCountryDropdownOpen() || isMobileMenuOpen() || isFocusedFieldInsideContactForm();
  }
  ...
  function blurOnKeyboardClose() {
    ...
    if (!vv) return;
    const h = vv.height;
    keyboardMinH = Math.min(keyboardMinH !== null && keyboardMinH !== void 0 ? keyboardMinH : h, h);
    ...
    if (keyboardWasOpen && h >= keyboardBaseH - 40 && Date.now() >= keyboardReadyAt) {
  ...
  function getEffectiveViewportTop() {
    ...
    const vvTop = Math.round((_window$visualViewpor = (_window$visualViewpor2 = window.visualViewport) === null || _window$visualViewpor2 === void 0 ? void 0 : _window$visualViewpor2.pageTop) !== null && _window$visualViewpor !== void 0 ? _window$visualViewpor : 0);
  ...
  function getEffectiveViewportHeight() {
    ...
    return Math.max(1, Math.round((_window$visualViewpor3 = (_window$visualViewpor4 = window.visualViewport) === null || _window$visualViewpor4 === void 0 ? void 0 : _window$visualViewpor4.height) !== null && _window$visualViewpor3 !== void 0 ? _window$visualViewpor3 : window.innerHeight));
  }
  ...
  function isSectionPlacementBroken(targetIdx) {
    ...
    if (rect && (targetIdx === 1 || rect.height <= viewportH * 1.15)) {
      ...
      if (Math.abs(breathCenter - viewportCenter) > SECTION_MISALIGN_PX) return true;
    }
  ...
  function armContactInputJumpProbe(field) {
    ...
    const contactBottom = Math.max(contactTop, Math.round(contactEl.offsetTop + contactEl.offsetHeight));
  ...
  function enableContactInputModalFallbackForProbe(reason = '') {
    ...
    if (field && (_window$contactInputM = window.contactInputModalFallback) !== null && _window$contactInputM !== void 0 && _window$contactInputM.openForField) {
  ...
  function detectContactInputJumpAndEnableFallback(reason = '') {
    ...
    if (!isFocusedFieldInsideContactForm()) return;
    const vvTop = Math.round((_window$visualViewpor5 = (_window$visualViewpor6 = window.visualViewport) === null || _window$visualViewpor6 === void 0 ? void 0 : _window$visualViewpor6.pageTop) !== null && _window$visualViewpor5 !== void 0 ? _window$visualViewpor5 : 0);
  ...
  function setActiveHeader(idx) {
    headerLinks.forEach(a => a.classList.toggle('is-active', indexOfAnchor(a) === idx));
  }
  let sideNav = null;
  let sideItems = [];
  function buildSideNav() {
    ...
    sections.forEach((sec, i) => {
      var _sec$querySelector;
      const li = document.createElement('li');
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'side-nav__btn';
      const label = sec.dataset.navTitle || ((_sec$querySelector = sec.querySelector('.section__title')) === null || _sec$querySelector === void 0 || (_sec$querySelector = _sec$querySelector.textContent) === null || _sec$querySelector === void 0 ? void 0 : _sec$querySelector.trim()) || `Section ${i + 1}`;
      ...
      btn.addEventListener('click', () => {
        ...
        if (id) history.pushState(null, '', id), window.dispatchEvent(new HashChangeEvent('hashchange'));
      });
      li.appendChild(btn);
      ul.appendChild(li);
      sideItems.push(btn);
  ...
  function computeFitScaleFor(el) {
    ...
    const availH = Math.max(0, window.innerHeight - HEADER_H - TOP_GAP - BOTTOM_GAP);
  ...
  async function lockAndGo(nextIndex) {
    var _sections$nextIndex;
    if (isSwiperGestureLocked()) return;
    if (nextIndex < 0 || nextIndex >= stops.length || locked || shouldPauseFullpage()) return;
    locked = true;
    current = nextIndex;
    setActive(nextIndex);
    const nextBreath = (_sections$nextIndex = sections[nextIndex]) === null || _sections$nextIndex === void 0 ? void 0 : _sections$nextIndex.querySelector('.section__breath');
    if (nextBreath) {
  ...
  (_window$visualViewpor7 = window.visualViewport) === null || _window$visualViewpor7 === void 0 || _window$visualViewpor7.addEventListener('resize', () => {
    blurOnKeyboardClose();
    detectContactInputJumpAndEnableFallback('vv-resize');
    if (!shouldPauseFullpage()) scheduleSectionPlacementRepair(120, 'vv-resize');
  });
...
//# sourceMappingURL=full-page-scroll.js.map