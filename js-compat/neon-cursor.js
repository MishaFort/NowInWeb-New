...
(() => {
  const DESKTOP_CURSOR_MEDIA = window.matchMedia('(min-width: 1140px) and (pointer: fine)');
  ...
  const onMouseMove = event => {
    ...
    const overScaling = target instanceof Element && !!target.closest('.js-no-cursor-trail');
  ...
  const renderTrail = () => {
    ...
    const sprayPoints = points.slice(1).filter(point => {
      const distanceToHead = Math.hypot(point.x - headPoint.x, point.y - headPoint.y);
      return distanceToHead > HEAD_CLEAR_RADIUS;
    });
    const circles = sprayPoints.map((point, index) => {
...
`;
  ...
  const checkMode = () => {
    if (DESKTOP_CURSOR_MEDIA.matches) start();else stop();
  };
  checkMode();
...
//# sourceMappingURL=neon-cursor.js.map