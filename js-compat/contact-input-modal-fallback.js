...
(() => {
  ...
  window.__contactInputModalModeEnabled = sessionStorage.getItem('contactInputModalMode') === '1';
  function isContactInputModalModeEnabled() {
    return window.__contactInputModalModeEnabled === true && window.innerWidth < INPUT_MODAL_MAX_W;
  }
  ...
  function isCountrySearchField(field) {
    var _field$classList;
    return !!(field !== null && field !== void 0 && (_field$classList = field.classList) !== null && _field$classList !== void 0 && _field$classList.contains('phone-input-wrapper__search-input'));
  }
  function getFieldLabel(field) {
    var _document$querySelect;
    if (!field) return 'Edit field';
    if (isCountrySearchField(field)) return 'Choose country';
    const label = field.id && ((_document$querySelect = document.querySelector(`label[for="${field.id}"]`)) === null || _document$querySelect === void 0 ? void 0 : _document$querySelect.textContent) || field.getAttribute('aria-label') || field.name || field.placeholder || 'Edit field';
  ...
  function createEditorFor(field) {
    ...
    if (isTextarea) {
      ...
      const allowedTypes = new Set(['text', 'email', 'tel', 'number', 'url', 'search', 'password']);
  ...
  function commitAndClose() {
    ...
    if (sourceField.value !== nextValue) {
      ...
      sourceField.dispatchEvent(new Event('input', {
        bubbles: true
      }));
  ...
  modal.addEventListener('keydown', e => {
    ...
    if (e.key === 'Enter' && editorField && editorField.tagName !== 'TEXTAREA') {
      e.preventDefault();
      commitAndClose();
    }
  });
...
//# sourceMappingURL=contact-input-modal-fallback.js.map