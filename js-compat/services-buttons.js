...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.services-section__item .button--arrow[href]').forEach(btn => btn.removeAttribute('href'));

  // Перевірка на мобільний пристрій
  const isMobileLike = () => window.matchMedia('(max-width: 1024px)').matches || window.matchMedia('(pointer: coarse)').matches;
  ...
  servicesList.addEventListener('click', e => {
    ...
    const titleEl = card === null || card === void 0 ? void 0 : card.querySelector('.services-card__title');
    const title = ((titleEl === null || titleEl === void 0 ? void 0 : titleEl.textContent) || '').replace(/\s+/g, ' ').trim();
    ...
    setTimeout(() => {
      var _window$fullpage;
      if ((_window$fullpage = window.fullpage) !== null && _window$fullpage !== void 0 && _window$fullpage.goToId) {
